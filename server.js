...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.cookie('access_token', data.session.access_token, {
      httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 86400000
    });
...
app.get('/api/chefs', authenticateToken, async (req, res) => {
  try {
    const { data, error } = await supabase.from('profiles').select('id, username, full_name, department').eq('role', 'chef');
...
// Récupérer tous les employés terrain
...
// Créer Utilisateur Système (Chef d'équipe)
